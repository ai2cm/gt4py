...
if __name__ == "__main__":
    ...
    if directory_existed and not parse_args.force:
        exit_failure("Directory already exists. Use --force or -f to overwrite directory. Exiting...", code=ExitCode.WRITE_ERROR)
    ...
    def get_git_object(section, remote):
        if "branch" in section:
            ...
            exit_failure(f"Feature sections require either \"branch\" or \"hash\" key", code=ExitCode.NONEXISTANT_KEY)
    ...
    for feature in features:
        ...
        remote_urls = {section.split()[1].strip("\"") : git_config[section]["url"]
            for section in
            filter(lambda key: "remote" in key, git_config.sections())}
        remote_name = None
        ...
        if run_git_cmd("ls-files", "-u").stdout:
            exit_failure("Merge conflict detected", code=ExitCode.MERGE_CONFLICT)
    ...
    run_git_cmd("add", os.path.split(script_in_directory)[-1], os.path.split(config_in_directory)[-1])
